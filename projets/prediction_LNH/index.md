...
**Ma contribution :** conception et implémentation du pipeline de préparation de données avancées pour l'apprentissage automatique (`process_data.py`) et développement complet de la modélisation par forêts aléatoires (`Foret_aleatoire.py`), incluant l'ingénierie de variables spécifiques, le traitement de la colinéarité, l'optimisation des hyperparamètres et l'analyse de performance. Le reste du rapport présente les travaux de l'équipe sur l'analyse descriptive, l'ACP et la régression linéaire.
...
Avant l'entraînement, une analyse matricielle a permis d'isoler les fortes colinéarités (notamment entre `PLAYMAKER_RATIO` et `SNIPER_RATIO` dont la somme vaut 1, ou encore les redondances entre buts, passes et points totaux). Le retrait des métriques redondantes au profit d'un indicateur synthétique clair (`LAST_P_PER_GP`) a permis de clarifier la structure du signal sans perte d'information.

![Détection de la colinéarité avant traitement](photos/Colinearite.png)
*Figure 10 : Matrice de corrélation avant nettoyage, mettant en évidence d'importantes redondances structurelles entre les variables de scoring et d'indices corporels.*

![Détection de la colinéarité après sélection des variables](photos/Colinearite_apres.png)
*Figure 11 : Matrice de corrélation après sélection, assurant un jeu de variables décorrélées et informatives pour la forêt aléatoire.*
...
![Grille des hyperparamètres du modèle](photos/params.png)
...
- **Performance globale :** R² = 0,22 avec une erreur absolue moyenne (**MAE**) de **13,36 rangs** et un **RMSE** de **16,21** sur le jeu de test.
- **Précision par fenêtres de tolérance :** 13,51 % des sélections sont prédites à ±3 rangs, 42,43 % à ±10 rangs et 62,43 % à ±16 rangs.

### Importance des variables

![Importance des 15 variables les plus prédictives](photos/Importance.png)
*Figure 13 : Classement des variables selon leur contribution à la réduction de l'impureté dans la forêt aléatoire.*

L'analyse de l'importance des variables confirme sans ambiguïté les conclusions de l'étude :
1. **L'efficacité contextualisée domine :** `NHLe_PPG` est de loin la variable la plus importante, prouvant qu'un point en ligue professionnelle senior (KHL, SHL) a une valeur décisionnelle bien supérieure au volume accumulé en ligue junior.
2. **Le contexte immédiat et la maturité :** `LAST_P_PER_GP` (production récente) et `EXACT_AGE_AT_DRAFT` (âge précis au jour près) complètent le podium, démontrant que la maturité relative lors de l'année d'éligibilité influence fortement le jugement des recruteurs.

### Analyse des résidus et comportement prédictif

![Comparaison entre vrais rangs et rangs prédits](photos/Regression.png)
*Figure 14 : Distribution des prédictions du modèle par rapport à la diagonale idéale y = x.*

Le graphique révèle un comportement typique des modèles en environnement bruité :
- **Attraction vers la moyenne :** Pour minimiser son erreur quadratique globale, l'algorithme concentre ses prédictions dans une bande médiane (autour du 32ᵉ rang) et ne formule aucune prédiction au-delà du 48ᵉ rang.
- **Imprévisibilité des choix tardifs :** L'absence d'information sur les critères qualitatifs (vision de jeu, éthique de travail, entrevues de dépistage) rend les sélections de fin de 2ᵉ ronde difficiles à séparer statistiquement des profils moyens.
